...
const thumbnails = document.querySelectorAll('.minitureStrip > img');
...
iframes = document.getElementsByClassName('doc');
...
function selectMiniatureAndOpenDoc() {
  ...
  $('html, body').animate(
    { scrollTop: '+=' + $('#GOTO').offset().top + 'px' },
    1
  );

  // Activate the iframe / embed window
  iframes.forEach((iframe) => {
    iframe.classList.remove('active');
  });
  numDoc = $(this)[0].dataset.docnum;
  numDoc = parseInt(numDoc);
  iframe = document.getElementById(`doc${numDoc}`);
  iframe.classList.add('active');

  // Indicate selected image
  $(thumbnails).removeClass('selected-image');
  this.classList.add('selected-image');
}

$('.minitureStrip img').click(selectMiniatureAndOpenDoc);
